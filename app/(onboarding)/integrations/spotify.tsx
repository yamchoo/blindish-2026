 ...
 * Dedicated screen for connecting Spotify account
 */
...
import { spotifyDataService } from '@/features/personality/services/spotifyDataService';
import { Colors } from '@/lib/constants/colors';
import { Typography, Spacing, BorderRadius } from '@/lib/constants/typography';

const INSIGHTS = [
  { icon: '🎵', text: 'Match based on music taste and concert-going habits' },
  { icon: '🎸', text: 'Discover shared playlists and favorite artists' },
  { icon: '💕', text: 'Connect through emotional expression in music' },
];
...
export default function SpotifyIntegrationScreen() {
  ...
  const handleConnect = async () => {
    ...
    if (result.success) {
      // Sync detailed Spotify data to database (don't block on this)
      spotifyDataService.syncDetailedDataToDatabase(user.id).catch(err => {
        console.error('Failed to sync detailed Spotify data (non-blocking):', err);
      });
  ...
  const handleModalContinue = () => {
    ...
    router.push('/(onboarding)/integrations/youtube');
  };
        ...
        >
          ...
          >
            ...
            <View style={styles.main}>
              ...
              >
                <SpotifyLogo size={72} color="#1DB954" />
              </Animated.View>

              <Text style={styles.title}>Connect Spotify</Text>

              <Text style={styles.description}>
                Let your music taste speak for you. We'll analyze your listening habits to find matches with similar vibes.
              </Text>
